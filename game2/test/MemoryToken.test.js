...
contract('Memory Token', (accounts) => {
  ...
  describe('token distribution', async () => {
    ...
    it('mints tokens', async () => {
      await token.mint(accounts[1], 'https://www.token-uri.com/nft')
      ...
      result = await token.tokenOfOwnerByIndex(accounts[1], 0)

      // Owner can see all tokens
      let balanceOf = await token.balanceOf(accounts[1])
      let tokenIds = []
      for (let i = 0; i < balanceOf; i++) {
        let id = await token.tokenOfOwnerByIndex(accounts[1], i)
